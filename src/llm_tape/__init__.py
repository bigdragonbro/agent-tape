"""
llm-tape — framework-agnostic record/replay test harness for LLM agents.
...
Quick start::
    ...
    # Record
    with llm_tape.record("tapes/my_run.tape.yaml"):
        await my_agent.run("summarize document.txt")

    # Replay + assert
    with llm_tape.replay("tapes/my_run.tape.yaml") as tape:
        await my_agent.run("summarize document.txt")
